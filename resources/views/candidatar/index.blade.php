                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('candidatar.update', ['candidatar' => $vaga->id]) }}"> 
                        @if (count($perguntas) > 0)
                            ...
                            <input type="hidden" name="vaga_id" value="{{ $vaga->id }}" id="vaga_id">
                                    ...
                                    @if ($pergunta->options)
                                        ...
                                        @foreach ($options as $option)
                                            @if ($pergunta->mult_resps)
                                                <input type="checkbox" class="form-check-input" name="respostas[{{ $pergunta->id }}][]" value="{{ $option }}" id="resposta_{{ $pergunta->id }}_{{ $loop->index }}"> {{ $option }}
                                            @else
                                                <input type="radio" class="form-check-input" name="respostas[{{ $pergunta->id }}]" value="{{ $option }}" id="resposta_{{ $pergunta->id }}_{{ $loop->index }}"> {{ $option }}
                                            @endif
                                        @endforeach
                                    @else
                                        <input type="text" class="form-control" name="respostas[{{ $pergunta->id }}]" id="resposta_texto_{{ $pergunta->id }}"><br>
                                    @endif
                                </div>
                            @endforeach
                        @else
                            @csrf
                            @method('PUT')
                            <input type="hidden" name="vaga_id" value="{{ $vaga->id }}" id="vaga_id_no_questions">
